...
{
    /*  // --- Step 1: Detect if the triangle roughly covers the entire image ---
    bool isFullCanvas =
        (std::min({V1.position.x, V2.position.x, V3.position.x}) <= 0.0f &&
         std::max({V1.position.x, V2.position.x, V3.position.x}) >= img.width - 1 &&
         std::min({V1.position.y, V2.position.y, V3.position.y}) <= 0.0f &&
         std::max({V1.position.y, V2.position.y, V3.position.y}) >= img.height - 1);

    int minX, maxX, minY, maxY;

    // --- Step 2: If full canvas, cover entire image ---
    if (isFullCanvas) {
        minX = 0;
        maxX = img.width - 1;
        minY = 0;
    ...
    // --- Step 3: Otherwise, restrict to triangle bounding box (optimized path) ---
    else {
        minX = std::max(0, (int)std::floor(std::min({V1.position.x, V2.position.x, V3.position.x})));
        maxX = std::min(img.width - 1, (int)std::ceil(std::max({V1.position.x, V2.position.x, V3.position.x})));
        minY = std::max(0, (int)std::floor(std::min({V1.position.y, V2.position.y, V3.position.y})));
        maxY = std::min(img.height - 1, (int)std::ceil(std::max({V1.position.y, V2.position.y, V3.position.y})));
    }
    */
...
// Texture texture{ texture_width, texture_height, text_pixels };
 ...
 std::vector<glm::vec3> brick1 = loadImage("blue2.bmp", w1, h1);
 std::vector<glm::vec3> brick2 = loadImage("orange2.bmp", w2, h2);
 std::vector<glm::vec3> brick3 = loadImage("green2.bmp", w3, h3);
   ...
   // moveAndDraw(image, origin, direction, distance, glm::vec3(1, 0, 0));
    ...
    for (int y = 0; y < height; y += blockH) {
        for (int x = 0; x < width; x += blockW) {
            // Randomly pick which brick "color" to use
           
            fillRandomBrick(image, x, y, blockW, blockH,
                brick1, w1, h1,
                brick2, w2, h2,
                brick3, w3, h3);
        }
